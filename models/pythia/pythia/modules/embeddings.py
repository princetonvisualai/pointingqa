...
class ImageEmbedding(nn.Module):
    ...
    def forward(self, image_feat_variable, question_embedding, image_dims, extra={}, context_embedding=None):
        # N x K x n_att
        if context_embedding is not None:
            attention = self.image_attention_model(
                image_feat_variable, question_embedding, image_dims, context_embedding
            )
        ...
        if 'att' in extra:
            att_reshape = extra['att'].unsqueeze(1)
