...
@registry.register_loss("logit_bce_wt")
class LogitWeightedBinaryCrossEntropy(nn.Module):
    ...
    def forward(self, sample_list, model_output):
        ...
        L_norms = torch.sum(targets, dim=1).repeat(targets.shape[1], 1).transpose(1, 0) + 1 # weights of positive classes

        L_prime_norms = torch.sum(1 - targets, dim=1).repeat(targets.shape[1], 1).transpose(1, 0) # weights of negative classes

        wt = torch.div(targets, L_norms) + torch.div(1 - targets, L_prime_norms)

        return torch.mean(torch.sum(loss * wt, dim=1))
...
class NLLLoss(nn.Module):
    ...
    def forward(self, sample_list, model_output):
        ...
        """
        # EDITED: adding softmax
        scores = model_output["scores"]
        targets = sample_list["targets"]
        _, idx = targets.max(dim=1)
        loss = F.cross_entropy(scores, idx, reduction="mean")
