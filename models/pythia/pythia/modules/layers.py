...
class ModalCombineLayer(nn.Module):
    def __init__(self, combine_type, img_feat_dim, txt_emb_dim, **kwargs):
        ...
        elif combine_type == "non_linear_element_multiply_three":
            self.module = NonLinearElementMultiplyThree(img_feat_dim, txt_emb_dim, img_feat_dim, **kwargs)
...
class NonLinearElementMultiplyThree(nn.Module):
    def __init__(self, image_feat_dim, ques_emb_dim, context_emb_dim, **kwargs):
        super(NonLinearElementMultiplyThree, self).__init__()
        self.fa_image = ReLUWithWeightNormFC(image_feat_dim, kwargs["hidden_dim"])
        self.fa_txt = ReLUWithWeightNormFC(ques_emb_dim, kwargs["hidden_dim"])
        self.fa_context = ReLUWithWeightNormFC(context_emb_dim, kwargs["hidden_dim"])

        self.dropout = nn.Dropout(kwargs["dropout"])
        self.out_dim = kwargs["hidden_dim"]

    def forward(self, image_feat, question_embedding, context_embedding):

        image_fa = self.fa_image(image_feat)
        question_fa = self.fa_txt(question_embedding)
        context_fa = self.fa_context(context_embedding)

        if image_fa.dim() > question_fa.dim():
            question_fa = question_fa.unsqueeze(1)
        if image_fa.dim() > context_fa.dim():
            context_fa = context_fa.unsqueeze(1)

        joint_feature = image_fa * context_fa
        joint_feature = joint_feature * question_fa
