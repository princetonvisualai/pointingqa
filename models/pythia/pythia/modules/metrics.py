...
class Accuracy(BaseMetric):
    ...
    **Key:** ``accuracy``
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        ...
        output, expected = output.squeeze(), expected.squeeze() # Remove any extra dimensions
...
@registry.register_metric("accuracy_2way")
class Accuracy_2way(BaseMetric):

    def __init__(self):
        super().__init__("accuracy_2way")
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        ...
        # binarize the expected labels
        expected_bin = (expected >= 15)

        # max class method
        '''
        output_bin = (output >= 15)

        correct = (expected_bin == output_bin).sum().float()
        total = len(expected_bin)
        ...
        obj_sum = torch.sum(output_probs[:, :15])
        part_sum = torch.sum(output_probs[:, 15:])

        output_bin = ((part_sum - obj_sum) > 0)
        correct = (expected_bin == output_bin).sum().float()
        total = len(expected_bin)
        value = correct / total
        return value
        ...
        '''
        obj_labels = torch.fmod(expected, 15)
        part_labels = obj_labels + 15
        obj_probs = output_probs.gather(1, obj_labels.view(-1, 1)).squeeze()
        part_probs = output_probs.gather(1, part_labels.view(-1, 1)).squeeze()

        output_bin = ((part_probs - obj_probs) > 0)
        correct = (expected_bin == output_bin).sum().float()                                                                                                   
        total = len(expected_bin)                                                                                                                              
        value = correct / total                                                                                                                                
        return value 
...
@registry.register_metric("vqamb_f1pt")
class VQAmb_F1_Point(BaseMetric):

    from sklearn import metrics
    import pandas as pd

    def __init__(self):
        super().__init__("vqamb_f1pt")

    def calculate(self, sample_list, model_output, *args, **kwargs):

        output, expected = model_output["scores"], sample_list["targets"]
        output, expected = output.squeeze(), expected.squeeze() # Remove any extra dimensions
        expected = expected.cpu()

        # convert scores to one-hot predictions
        ...
        # combine predictions for each QA pair (pandas groupby)
        qa_inds = torch.Tensor(sample_list["qa_index"]).squeeze()
        one_hot_df, targets_df = self.pd.DataFrame(one_hot), self.pd.DataFrame(expected)
        
        one_hot_df["qa_index"] = qa_inds
        targets_df["qa_index"] = qa_inds

        one_hot_final = (one_hot_df.groupby("qa_index").sum().to_numpy() > 0).astype(float)
        targets_final = (targets_df.groupby("qa_index").sum().to_numpy() > 0).astype(float)

        f1_score = self.metrics.f1_score(targets_final, one_hot_final, average='micro')
        return f1_score

@registry.register_metric("vqamb_map")
class VQAmbmAPMetric(BaseMetric):
    """Metric for calculating mAP for VQAmb task.

    **Key:** ``vqamb_map``
    """
    from sklearn import metrics
    # metrics import average_precision_score
    
    def __init__(self):
        super().__init__("vqamb_map")
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        ...
        score = self.metrics.average_precision_score(expected.numpy(), output.detach().numpy(), average=None)
        
        mAP = np.mean(score[~np.isnan(score)])
        return mAP

@registry.register_metric("vqamb_f1")
class VQAmbF1Metric(BaseMetric):

    from sklearn import metrics

    def __init__(self):
        super().__init__("vqamb_f1")

    def find_thresholds(self, preds, target, metric_fn):

        thresholds = [x/20 for x in list(range(1, 20, 1))]
        thresh_ans = torch.zeros(target.shape[1])
        for ans in range(target.shape[1]):
            max_metric = 0
            max_thresh = 0
            for thresh in thresholds:
                metric = metric_fn(target[:, ans], (preds[:, ans] >= thresh).float())
                if metric > max_metric:
                    max_metric = metric
                    max_thresh = thresh

            thresh_ans[ans] = max_thresh

        return thresh_ans

    def thresh_per_class(self, output, thresholds):
        # preds = torch.sigmoid(output)
        preds = output.clone()
        for ans in range(preds.shape[1]):
            preds[:, ans] = (preds[:, ans] >= thresholds[ans]).float()
        return preds

    def calculate(self, sample_list, model_output, *args, **kwargs):

        output, expected = model_output["scores"], sample_list["targets"]
        output, expected = output.squeeze(), expected.squeeze()
        output = F.sigmoid(output).cpu()
        expected = expected.cpu()

        thresholds = self.find_thresholds(output, expected, self.metrics.f1_score)
    

        preds = self.thresh_per_class(output, thresholds)
        f1_score = self.metrics.f1_score(expected, preds, average='micro')
        
        # print thresholds if validation
        if sample_list.dataset_type != "train" and output.shape[0] > 128:
            print(thresholds)
            torch.save(preds, 'preds_pythia.pt')
            torch.save(expected, 'target_pythia.pt')

        preds = self.thresh_per_class(output, thresholds)
        f1_score = self.metrics.f1_score(expected, preds, average='micro')
        # f1_score = self.metrics.accuracy_score(expected, preds)
        return f1_score
